...
const textEmailRelease = ({ release, i }) =>
	`\t${i + 1}. \"${release.title}\" by ${release.artists.join(', ')} (${dateFormatted(
		release.date
	)})`
...
There ${
		releases.length === 1 ? 'is a new release' : `are the new releases`
	} available from your followed artists:
...
const HtmlEmailTemplate = ({ name, releases }) => (
	<div>
		<p>Hi {name},</p>
		<p>
			There {releases.length === 1 ? 'is a new release' : 'are new releases'} available from your
			followed artists:
		</p>
